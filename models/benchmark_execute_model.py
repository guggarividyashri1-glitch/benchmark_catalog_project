...
def validate_number(value, field_name: str):
    if not isinstance(value, int):
        raise ValueError(f"{field_name} must contain numbers only (no alphabets or special characters)")
    return value
def not_empty_dict(value: Dict, field_name: str):
    if value is None or len(value) == 0:
        raise ValueError(f"{field_name} is required and should not be empty")
    return value
